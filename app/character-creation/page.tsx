...
export default function CharacterCreation() {
  ...
  const [currentGender, setCurrentGender] = useState("girl")

  useEffect(() => {
    // Check if user came from gender selection
    const selectedGender = localStorage.getItem("selectedCompanionGender")
    if (selectedGender) {
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      localStorage.setItem("alterraActiveChatId", newProfile.id)
      localStorage.removeItem("selectedCompanionGender")
  ...
  return (
    <div className="h-screen w-full bg-animated-sunset flex items-center justify-center p-6 overflow-y-auto">
      <div className="w-full max-w-md mx-auto py-8">
        ...
        <div className="setup-card p-6 rounded-2xl">
          ...
          <button
            ...
            disabled={!isValid}
            className={`btn-primary w-full py-3 rounded-lg text-lg font-semibold mt-6 transition-all ${!isValid ? "opacity-50 cursor-not-allowed" : "hover:scale-105"}`}
          >
